...
HaDateTimePicker.prototype.renderDate = function(date) {
    ...
    //اصلاح باگ محاسبه در سال های کبیسه - رادان
    var kabisehYears = [1403, 1407, 1411, 1415, 1419, 1421];
    if (kabisehYears.includes(date.getFullYear())) {
        if (c <= 0) {
            c++;
        }
        else {
    ...
    for (var x = 0; x < weeks.length; x++) {
        ...
        for (var l = 0; l < weeks[x].daysOfWeek.length; l++) {
            ...
            if (dayOfMonth <= monthLastDay && dayOfMonth > 0) {
                ...
                if (dayOfMonth < 1) {
                    //if (date.getMonth() != 0 || (date.getMonth() == 0 && (previousMonthLastDay + dayOfMonth) != 30)) {
